...
func main() {
	...
	if len(os.Args) != 4 {
		fmt.Println("Invalid Number Command line Arguments")
		os.Exit(1)
	}
	udpAddr_Local,_ := net.ResolveUDPAddr("udp",os.Args[1])
	tcpAddr_Local,_ := net.ResolveTCPAddr("tcp",os.Args[2])
	udpAddr_Aserver,_ := net.ResolveUDPAddr("udp",os.Args[3])


	fmt.Println(udpAddr_Local,tcpAddr_Local,udpAddr_Aserver)
